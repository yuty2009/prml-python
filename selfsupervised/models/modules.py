...
class TransformerEncoderLayer(nn.Module):
    def __init__(self, d_model, n_heads, d_ff, dropout=0.1, attn_type='full'):
        super().__init__()
        ...
        if self.attn_type == 'full':
            ...
            self.attn = NystromAttention(
                ...
                heads = n_heads,
                num_landmarks = d_model // 2, # number of landmarks
                pinv_iterations = 6, # 6 was recommended by the paper
                residual = True, # supposedly faster convergence if turned on
                dropout = dropout
            )
        else:
...
class TransformerEncoderLayerTimm(nn.Module):
    def __init__(self, d_model: int, n_heads: int, d_ff: int, dropout: float = 0.1,
                 drop_path: float = 0., norm_layer=nn.LayerNorm, attn_type='full'):
        super().__init__()
