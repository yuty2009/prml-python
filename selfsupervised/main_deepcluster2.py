...
def main(gpu, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        assignments = cluster_memory(
            ...
            len(train_loader.dataset), args.feature_dim, args.n_prototypes, args=args)
...
def init_memory(data_loader, model, args):
    ...
    mb_embeddings = torch.zeros(len(args.crops_for_assign), mb_size, args.feature_dim).to(args.device)
...
def cluster_memory(
    model, mb_index, mb_embeddings,
    n_data, feature_dim, n_prototypes, n_iters=10, args=None):
    real_model = model.module if args.ngpus > 1 else model
    j = 0
