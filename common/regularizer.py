...
class GroupLassoRegularizer(Regularizer):
    ...
    """
    def __init__(self, groups=0, lambda_reg=0.01):
    ...
    def add_param(self, param):
        if len(param.shape) == 1:
            groupid = np.unique(self.groups)
            NG = len(groupid)
            loss_group = 0
            for g in range(NG):
                index_ig = np.argwhere(self.groups == groupid[g])
                loss_group += param[index_ig].norm(2, dim=0)
            loss_reg = self.lambda_reg * loss_group
        else:
            # The 2nd dimension is the output dimension in multi-output conditions
            loss_reg = self.lambda_reg * param.norm(2, dim=1).sum()
        return loss_reg
